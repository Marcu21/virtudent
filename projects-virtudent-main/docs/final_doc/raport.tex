...
To strategically assess the project's viability and future potential, we conducted a comprehensive SWOT analysis evaluating internal technical capabilities and external factors.

\subsection{Strengths (Internal)}
\begin{itemize}
    \item \textbf{Proven Logic Viability (Web Integration):} The core simulation logic proved to be robust and architecture-independent. The algorithmic module was successfully integrated into a live web application by a partner team, validating the system's modularity.
    \item \textbf{Rigorous Model Selection:} The systematic testing of multiple architectures (TinyLlama, Phi-2, LLaMA) ensures the final solution is empirically validated rather than arbitrarily chosen.
    \item \textbf{Robustness via Slot Filling:} Unlike standard chatbots, the system tracks symptom status in real-time, ensuring the patient remains consistent throughout the dialogue.
    \item \textbf{Ground Truth Enforcement:} The architecture strictly adheres to the provided knowledge base, effectively preventing hallucinations.
    \item \textbf{Dual Simulation:} The project offers a complete educational loop: training via the \textit{Patient Persona} and immediate validation via the \textit{Professor Persona}.
\end{itemize}

\subsection{Weaknesses (Internal)}
\begin{itemize}
    \item \textbf{Connectivity Dependency:} The logic relies entirely on the Groq API connection. Network instability directly impacts the fluidity of the conversation.
    \item \textbf{Static Database Management:} Adding new clinical cases requires manual editing of Excel files rather than using a dynamic administrative interface.
    \item \textbf{Limited Coverage:} Currently restricted to 8 dental pathologies and a relatively small training dataset (225 conversations), which limits exposure to rare cases.
    \item \textbf{Language Specificity:} The system is primarily optimized for Romanian, limiting immediate international applicability without retraining.
\end{itemize}

\subsection{Opportunities (External)}
\begin{itemize}
    \item \textbf{Native Mobile Application:} Wrapping the core logic into a native mobile application (iOS/Android) would offer a smoother experience compared to the current web interface.
    \item \textbf{Multimodal Integration:} Extending the dataset to include imaging data (X-rays, intraoral photos) linked to specific Excel cases would complete the clinical picture.
    \item \textbf{Gamification:} Introducing a scoring system based on diagnostic efficiency (e.g., reaching the correct diagnosis with fewer questions yields a higher score).
\end{itemize}

\subsection{Threats (External)}
\begin{itemize}
    \item \textbf{Costs and API Changes:} The project depends on an external provider (Groq/Llama). Future pricing changes or API deprecations would require code maintenance.
    \item \textbf{Educational Risks regarding Data Accuracy:} Since the AI strictly respects the provided data, any medical errors in the "Ground Truth" (Excel files) would be propagated to students. Rigorous verification is mandatory.
\end{itemize}
